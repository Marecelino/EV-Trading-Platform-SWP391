...
export const ComparisonProvider = ({ children }: { children: ReactNode }) => {
  ...
  const addItem = (product: Product) => {
    ...
    if (items.length >= MAX_ITEMS || items.find(item => item._id === product._id)) {
    ...
    if (items.length > 0) {
        ...
        if (isEv !== productIsEv) {
            alert("Vui lòng chỉ so sánh các sản phẩm cùng danh mục (Xe điện hoặc Pin).");
            return;
        }
    }
  ...
  return (
    <ComparisonContext.Provider value={{ items, addItem, removeItem, isInCompare }}>
      {children}
