...
export class BatteryListingsService {
    ...
    async create(dto: any) {
        ...
        if (payload['seller_id'] && payload['title']) {
            ...
            if (recent) {
                ...
                if (age < 10000) {
                    const batteryDetail = await this.batteryDetailModel.findOne({ listing_id: recent._id }).lean();
        ...
        Object.keys(batteryPayload).forEach((k) => batteryPayload[k] === undefined && delete batteryPayload[k]);
        const batteryDetail = await this.batteryDetailModel.create(batteryPayload);

        // Trả về cả listing và batteryDetail
        return { listing: saved.toObject(), batteryDetail: batteryDetail.toObject() };
    }
    ...
    async update(id: string, dto: any) {
        ...
        if (Object.keys(batPayload).length > 0) {
            batteryDetail = await this.batteryDetailModel.findOneAndUpdate(
                { listing_id: id },
                { $set: batPayload },
                { upsert: true, new: true, setDefaultsOnInsert: true },
            ).lean();
        } else {
            batteryDetail = await this.batteryDetailModel.findOne({ listing_id: id }).lean();
        }
