    ...
    await new Promise<void>((resolve, reject) => {
      ...
      try {
        ...
        if (chosen) {
          ...
          console.warn(
            'No Unicode TTF font found in ./BE/fonts. Vietnamese characters may render incorrectly. Place NotoSans-Regular.ttf or Times New Roman TTF into BE/fonts/ to fix.',
          );
      ...
      doc
        .fontSize(12)
        .text(`Số hợp đồng: ${(contract as any).contract_no || id}`);
      ...
      const buyerName =
        transaction?.buyer_id?.name || (contract as any).buyer_name || '';
      const sellerName =
        transaction?.seller_id?.name || (contract as any).seller_name || '';
      const amount =
        transaction?.price ||
        transaction?.amount ||
        (contract as any).amount ||
        '';
      ...
      const terms =
        (contract as any).terms_and_conditions || 'Không có điều khoản.';
      ...
      if (isSigned) {
        ...
        const signedAt = (contract as any).signed_at
          ? new Date((contract as any).signed_at)
          : signerFound?.at
