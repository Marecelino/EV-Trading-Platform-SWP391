...
export class BatteryAuctionService {
    constructor(
        ...
        @InjectModel(BatteryDetail.name) private readonly batteryDetailModel: Model<any>,
    ...
    async create(dto: any) {
        ...
        if (Number.isNaN(starting) || starting < 0) {
            throw new BadRequestException('starting_price must be a non-negative number');
        }
        ...
        Object.keys(batPayload).forEach((k) => batPayload[k] === undefined && delete batPayload[k]);

        const update: any = { $set: { ...batPayload, auction_id: saved._id } };
        if (dto.listing_id) update.$set.listing_id = dto.listing_id;

        const batteryDetail = await this.batteryDetailModel.findOneAndUpdate(
            { $or: selectorClauses },
            update,
    ...
    async update(auctionId: string, dto: any) {
        ...
        const listingId = (auction as any).listing_id?.toString();
        if (!listingId) throw new BadRequestException('Auction has no linked listing');
        ...
        const updatedListing = await this.listingModel.findByIdAndUpdate(listingId, updateListingPayload, { new: true, runValidators: true }).lean();
        if (!updatedListing) throw new NotFoundException('Linked listing not found');

        const batPayload: any = {};
        if (dto.capacity_kwh !== undefined) batPayload.capacity_kwh = dto.capacity_kwh;
        if (dto.battery_capacity !== undefined) batPayload.capacity_kwh = dto.battery_capacity;
        ...
        if (Object.keys(batPayload).length > 0) {
            ...
            await this.batteryDetailModel.findOneAndUpdate(
                { $or: selectorClauses },
                update,
                { upsert: true, new: true, setDefaultsOnInsert: true },
            ).exec();
        ...
        if (dto.start_time) updateAuctionPayload.start_time = new Date(dto.start_time);
        if (dto.end_time) updateAuctionPayload.end_time = new Date(dto.end_time);
        if (dto.starting_price !== undefined) updateAuctionPayload.starting_price = dto.starting_price;
        if (dto.min_increment !== undefined) updateAuctionPayload.min_increment = dto.min_increment;
        if (dto.buy_now_price !== undefined) updateAuctionPayload.buy_now_price = dto.buy_now_price;
        ...
        const updatedAuction = await this.auctionModel.findByIdAndUpdate(auctionId, updateAuctionPayload, { new: true, runValidators: true }).lean();
