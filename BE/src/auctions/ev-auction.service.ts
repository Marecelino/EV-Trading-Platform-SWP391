...
export class EVAuctionService {
    ...
    async create(dto: any) {
        ...
        const evDetail = await this.evDetailModel.findOneAndUpdate(
            { $or: selectorClauses },
            update,
    ...
    async update(id: string, dto: any) {
        ...
        const auction = await this.auctionModel.findByIdAndUpdate(id, updatePayload, { new: true, runValidators: true }).lean();
        ...
        if (Object.keys(evPayload).length > 0) {
            ...
            if ((auction as any).listing_id) selectorClauses.push({ listing_id: (auction as any).listing_id.toString() });

            const update: any = { $set: { ...evPayload, auction_id: id } };
            if ((auction as any).listing_id) update.$set.listing_id = (auction as any).listing_id;

            await this.evDetailModel.findOneAndUpdate(
                { $or: selectorClauses },
                update,
                { upsert: true, new: true, setDefaultsOnInsert: true },
            ).exec();
